...
export const ModuleServiceActionsTab: React.FC<ModuleServiceActionsTabProps> = ({ rawData }) => {
  ...
  const serviceActions = ensureArray(serviceActionsRaw);

  const filteredServiceActions = serviceActions.filter((act: any) => {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Cari service action berdasarkan nama atau deskripsi..."
            value={search}
      ...
      {/* Main Table */}
      {filteredServiceActions.length === 0 ? (
        <div className="py-12 px-4 bg-surface rounded-card border border-dashed border-outline text-center space-y-2">
          ...
          <p className="text-sm font-bold text-gray-700">Tidak ada Service Action ditemukan</p>
          <p className="text-xs text-gray-500">Modul ini tidak mengekspos Service Action atau tidak cocok dengan filter pencarian.</p>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-outline/60 text-xs">
                {filteredServiceActions.map((act: any, idx: number) => {
